...
SaveData::SaveData() {
	...
	if (stat(FILENAME, &buf) == -1) {
		...
		writeFile << this->save_file; //Save XML to file
		...
		//Create file buffer
		vector<char> buffer((istreambuf_iterator<char>(this->readFile)), istreambuf_iterator<char>()); //Create buffer
...
SaveData::~SaveData() {
	...
	//Free space
	save_file.clear(); //Destroy document to free space

	readFile.close(); //Close fstream's
	writeFile.close();

}

void SaveData::saveData(string arena, Player player) {
